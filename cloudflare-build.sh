...
printf 'PROJECT SOVEREIGN Cloudflare build v0.6.0\n'
...
node -e "const p=require('./package.json'); if(p.version!=='0.6.0') { console.error('Unexpected package version:', p.version); process.exit(1); }"
if [[ -f public/_redirects ]]; then
  echo 'ERROR: public/_redirects remains after applying v0.6.' >&2
  exit 1
fi
...
for marker in \
  ...
  if ! grep -R -q -- "$marker" "$OUTPUT"; then
    echo "ERROR: v0.6 deployment marker is missing: $marker" >&2
    exit 1
  fi
done
...
printf 'Deployment output verified: %s files; v0.6 tests and build passed.\n' "$(find "$OUTPUT" -type f | wc -l | tr -d ' ')"
